...
#runs <- read.table("170613-151604-BaseValues-filterOn.txt", sep="\t", header=TRUE)
...
runs.sent <- read.table("170613-151604-Packets.txt", sep="\t", header=TRUE)
...
#filter.sent.f <- factor(runs.sent$FILTER, levels=c("Ideal"))

sent = ggplot(runs.sent, aes(x=sensdist.sent.f, y = runs.sent$PACKETS_SENT)) +
...
cat("before error\n")
runs.edr <- read.table("170613-151604-Determinants.txt", sep="\t", header=TRUE)
...
edr = ggplot(mlt, aes(x=sensdist.edr.f, y = mlt$value)) +
...
runs.fail <- read.table("170613-151604-RX.txt", sep="\t", header=TRUE)
...
#filter.fail.f <- factor(runs.fail$FILTER, levels=c("Ideal"))

failure = ggplot(runs.fail, aes(x=sensdist.fail.f, y = (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX) / (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX + runs.fail$RECEIVED_DESPITE_INT + runs.fail$RECEIVED_NO_INT))) +
...
p2 <- ggplot(runs.fail, aes(x=sensdist.fail.f, y = runs.fail$FAILED_NO_INT / (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX))) +
            geom_boxplot(notch=TRUE) +
            coord_cartesian(ylim = c(0.0, 1.0)) +
...
p3 <- ggplot(runs.fail, aes(x=sensdist.fail.f, y = runs.fail$FAILED_WITH_INT / (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX))) +
            geom_boxplot(notch=TRUE) +
            coord_cartesian(ylim = c(0.0, 1.0)) +
...
p4 <- ggplot(runs.fail, aes(x=sensdist.fail.f, y = runs.fail$FAILED_BELOW_SENS / (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX))) +
            geom_boxplot(notch=TRUE) +
            coord_cartesian(ylim = c(0.0, 1.0)) +
...
p5 <- ggplot(runs.fail, aes(x=sensdist.fail.f, y = runs.fail$FAILED_NON_RX / (runs.fail$FAILED_NO_INT + runs.fail$FAILED_WITH_INT + runs.fail$FAILED_BELOW_SENS + runs.fail$FAILED_NON_RX))) +
            geom_boxplot(notch=TRUE) +
            coord_cartesian(ylim = c(0.0, 1.0)) +
...
#runs.overflows <- read.table("170613-151604-BO.txt", sep="\t", header=TRUE)
...
#overflows = ggplot(mlt, aes(x=sensdist.overflows.f, y = mlt$value)) +
