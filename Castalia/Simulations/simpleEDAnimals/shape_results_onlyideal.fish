...
sed -i.bak -e '1 s/.*/NUMSOURCES	SENSINGDISTANCE	RUN	EVENTS	FALSE_NEGATIVES	FALSE_POSITIVES	NEGATIVES	PACKETS_SENT	POSITIVES	TRUE_NEGATIVES	TRUE_POSITIVES/' $infile-BaseValues.txt
...
#cp $infile-BaseValues.txt $infile-BaseValues-filterOn.txt
...
sed -i.bak -e '/^[0-9]\+\s\+[0-9]\+\s\+[0-9]\+\s\+[0-9]\+\s\+$/ d' $infile-Packets.txt
...
sed -i.bak -e '1 s/.*/NUMSOURCES	SENSINGDISTANCE	RUN	FAILED_NO_INT	FAILED_WITH_INT	FAILED_BELOW_SENS	FAILED_NON_RX	RECEIVED_DESPITE_INT	RECEIVED_NO_INT/' $infile-RX.txt
sed -i.bak -e 's/ /\t/g' $infile-RX.txt
sed -i.bak -e 's/-/ /g' $infile-RX.txt
gawk -i inplace 'BEGIN{FS="\t";OFS="\t";} {print $3,$1,$2,$4,$5,$6,$7,$8,$9}' $infile-RX.txt
...
sed -i.bak -e '1 s/.*/NUMSOURCES	RUN	SENSDISTANCE30	SENSDISTANCE40	SENSDISTANCE50/' $infile-BO.txt
sed -i.bak -e 's/ /\t/g' $infile-BO.txt
sed -i.bak -e 's/-/ /g' $infile-BO.txt
gawk -i inplace 'BEGIN{FS="\t";OFS="\t";} {print $2,$1,$3,$4,$5}' $infile-BO.txt
...
sed -i.bak -e '1 s/.*/NUMSOURCES	RUN	SENSDISTANCE30	SENSDISTANCE40	SENSDISTANCE50/' $infile-Determinants.txt
sed -i.bak -e 's/ /\t/g' $infile-Determinants.txt
sed -i.bak -e 's/-/ /g' $infile-Determinants.txt
gawk -i inplace 'BEGIN{FS="\t";OFS="\t";} {print $2,$1,$3,$4,$5}' $infile-Determinants.txt
echo "Produced file: $infile-Determinants.txt"

cat newplots_onlyideal.r | string replace -r '[0-9][0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9][0-9][0-9]' $infile > newplots_onlyideal.r.bak
mv newplots_onlyideal.r.bak newplots_onlyideal.r
Rscript newplots_onlyideal.r
