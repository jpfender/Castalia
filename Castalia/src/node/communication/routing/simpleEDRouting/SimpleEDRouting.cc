...
#include "SimpleEDRouting.h"

Define_Module(SimpleEDRouting);

void SimpleEDRouting::startup()
{
	...
	if (appModule->hasPar("isSink"))
		...
		opp_error("\nSimpleED routing has to be used with an application that defines the parameter isSink");
...
void SimpleEDRouting::sendTopologySetupPacket()
{
	SimpleEDRoutingPacket *setupPkt =
	    new SimpleEDRoutingPacket("SimpleED routing setup packet", NETWORK_LAYER_PACKET);
	setupPkt->setSimpleEDRoutingPacketKind(MPRINGS_TOPOLOGY_SETUP_PACKET);
...
void SimpleEDRouting::sendControlMessage(simpleEDRoutingControlDef kind)
{
	SimpleEDRoutingControlMessage *ctrlMsg =
	    new SimpleEDRoutingControlMessage("SimpleED routing control message",NETWORK_CONTROL_MESSAGE);
	ctrlMsg->setSimpleEDRoutingControlMessageKind(kind);
	ctrlMsg->setLevel(currentLevel);
	ctrlMsg->setSinkID(currentSinkID);
	toApplicationLayer(ctrlMsg);
}

void SimpleEDRouting::timerFiredCallback(int index)
{
...
void SimpleEDRouting::processBufferedPacket()
{
...
void SimpleEDRouting::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	string dst(destination);

	SimpleEDRoutingPacket *netPacket =
	    new SimpleEDRoutingPacket("SimpleED routing data packet", NETWORK_LAYER_PACKET);
	netPacket->setSimpleEDRoutingPacketKind(MPRINGS_DATA_PACKET);
...
void SimpleEDRouting::fromMacLayer(cPacket * pkt, int macAddress, double rssi, double lqi)
{
	SimpleEDRoutingPacket *netPacket = dynamic_cast <SimpleEDRoutingPacket*>(pkt);
	if (!netPacket)
		return;

	switch (netPacket->getSimpleEDRoutingPacketKind()) {
